...
* **Automated Docker testing framework** using Docker containers, if docker is available.
  ...
  * **Cyrus CalDAV initialization**: Increased timeout for Cyrus CalDAV access verification in GitHub Actions from 60s to 120s, as CalDAV initialization can take significant time after the HTTP server is ready, especially in CI environments.
