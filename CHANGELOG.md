...
* `response.py`: XML parser lacked `resolve_entities=False` and `no_network=True`, leaving entity expansion and DTD network fetches enabled by default.  A malicious or MITM server could inject arbitrary text into parsed property values via inline DOCTYPE entity definitions.  Fixed by adding `resolve_entities=False, no_network=True` to `etree.XMLParser`.
