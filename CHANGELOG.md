...
* New compatibility feature `url.encode-at` (default `unknown`): whether the server resolves a resource whose path contains a literal `@` when that `@` is sent percent-encoded as `%40`.  RFC3986 allows `@` literally in a path segment (section 3.3) and does not license decoding `%40` back to `@` (section 6.2.2.2), so the two spellings are formally distinct and servers disagree about whether they name the same resource — which matters wherever a path embeds an email-like identifier, e.g. an ownCloud/Nextcloud calendar-home-set (`/remote.php/dav/calendars/user@example.com/`) or an object whose UID is an email address.  `full` = `%40` resolves, `unsupported` = it does not and the client must send the literal `@`.  Whether the literal `@` *fails*, so that encoding is forced, is a separate fact with its own key, `url.encode-at-required` (default `unknown`) — a server can accept both spellings, exactly one, or neither, and the dashed key keeps a declared parent from being inherited where it would invert the meaning.  `quirk` on either key is a severity rather than a behaviour: it records that the server deviates, and the profile entry should carry a `behaviour` string saying how, since another implementation's `@` quirk may be something else entirely.  Left at the `unknown` default the client behaves exactly as before, so nothing changes for a server you have not configured this for.  Probed by caldav-server-tester.
...
* New feature `non-existing-raises-not-found.collection`: whether a lookup of a non-existing *calendar collection* raises `NotFoundError`, declarable apart from the object-level lookup its parent describes.  The two differ in practice.  Robur answers 403 for everything that does not exist, objects included, but `CalendarObjectResource.load()` retries a failed GET as a calendar-multiget REPORT against the existing parent calendar, where Robur reports the missing href with an inner 404 - so an object lookup still ends in `NotFoundError` while a calendar lookup surfaces `AuthorizationError`.  Robur's profile now declares exactly that, replacing a `non-existing-raises-not-found: fragile` that had to skip the comparison altogether.
