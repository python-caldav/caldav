...
* New compatibility feature `url.encode-at` (default `unknown`): how a server treats a literal `@` in a resource path versus its percent-encoded spelling `%40`.  RFC3986 §3.3 makes `@` a legal `pchar`, so a producer never has to encode it, while §2.2 makes it *reserved*, so the two spellings are formally not equivalent (§6.2.2.2 licenses decoding only *unreserved* octets) — and servers disagree.  It bites wherever a path embeds an email-like identifier: an ownCloud/Nextcloud calendar-home-set (`/remote.php/dav/calendars/user@example.com/`), or an object whose UID is an email address.  Two extra keys carry the facts, and the URL code reads only those — the support level is a severity and nothing more.  `at-spelling` says which spellings resolve (`literal`, `encoded`, `both`); `at-identity` says, when both do, whether they name one resource (`aliased`) or two (`distinct`).  `distinct` is the conformant reading and the dangerous one: the library rewrites spellings, so on such a server it would write one resource and read another, silently.  `_quote_url_path()`, `_sanitize_calendar_home_set_url()` and `_quote_uid()` act on this; with neither key declared — the normal case — every one of them keeps the heuristic it had before the feature existed, so nothing changes for an unprobed server.  An unrecognised value is ignored rather than guessed at, so a typo in a profile cannot silently rewrite URLs.  `caldav-server-tester` probes it.  Not covered here: a server that stores under one spelling and reports back the other, which is a flavour of `save-load.stable-url`.
