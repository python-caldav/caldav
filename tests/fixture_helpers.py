...
async def _get_or_create_impl(
    ...
    ## same dict also carries bookkeeping like "support" and "cleanup-regime", and
    ## "cal_url" is spelled "cal_id" there (it takes a URL as well as an id).
    lookup = {k: v for k, v in test_cal_info.items() if k in ("name", "cal_id")}
    if "cal_url" in test_cal_info:
        lookup.setdefault("cal_id", test_cal_info["cal_url"])
    if lookup:
...
async def afix_calendar(
    ...
    * the display name is dropped on servers that cannot set one, or that move
      the calendar to a server-chosen URL when one is set - same reasoning as
      ``_fixCalendar_``: the fixture is always looked up by ``cal_id``.
    * whatever is left inside the calendar is wiped, so the caller can count
    ...
    if calendar_name is not None and not (
        _supports(client, "create-calendar.set-displayname")
        and _supports(client, "create-calendar.stable-url")
    ):
        calendar_name = None
