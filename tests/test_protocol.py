...
class TestCalDAVProtocol:
    ...
    def test_init_with_credentials(self):
        """Protocol should store credentials and build auth header."""
        protocol = CalDAVProtocol(
            base_url="https://cal.example.com",
            username="user",
            password="pass",
        )
        assert protocol.base_url == "https://cal.example.com"
        assert protocol._auth_header is not None
        assert protocol._auth_header.startswith("Basic ")

    def test_init_without_credentials(self):
        """Protocol without credentials should have no auth header."""
        protocol = CalDAVProtocol(base_url="https://cal.example.com")
        assert protocol._auth_header is None

    def test_resolve_url_relative(self):
        """Relative paths should be resolved against base URL."""
        protocol = CalDAVProtocol(base_url="https://cal.example.com")
        assert protocol._resolve_url("/calendars/") == "https://cal.example.com/calendars/"

    def test_resolve_url_absolute(self):
        """Absolute URLs should be returned unchanged."""
        protocol = CalDAVProtocol(base_url="https://cal.example.com")
        assert protocol._resolve_url("https://other.com/cal/") == "https://other.com/cal/"

    def test_propfind_request(self):
        """propfind_request should build correct request."""
        protocol = CalDAVProtocol(base_url="https://cal.example.com")
        request = protocol.propfind_request("/calendars/", ["displayname"], depth=1)

        assert request.method == DAVMethod.PROPFIND
        assert request.url == "https://cal.example.com/calendars/"
        assert request.headers["Depth"] == "1"
        assert request.body is not None

    def test_calendar_query_request(self):
        """calendar_query_request should build correct request."""
        protocol = CalDAVProtocol(base_url="https://cal.example.com")
        ...
        assert request.method == DAVMethod.REPORT
        assert "cal.example.com" in request.url
        assert request.headers["Depth"] == "1"

    def test_put_request(self):
        """put_request should build correct request with body."""
        protocol = CalDAVProtocol(base_url="https://cal.example.com")
        ical_data = b"BEGIN:VCALENDAR\nEND:VCALENDAR"
        request = protocol.put_request(
            "/calendars/user/event.ics",
            data=ical_data,
            etag='"old-etag"',
        )

        assert request.method == DAVMethod.PUT
        assert request.headers["If-Match"] == '"old-etag"'
        assert request.body == ical_data

    def test_delete_request(self):
        """delete_request should build correct request."""
        protocol = CalDAVProtocol(base_url="https://cal.example.com")
        request = protocol.delete_request("/calendars/user/event.ics")

        assert request.method == DAVMethod.DELETE
        assert "event.ics" in request.url

    def test_parse_propfind(self):
        """parse_propfind should delegate to parser."""
        protocol = CalDAVProtocol()
        xml = b"""<?xml version="1.0"?>
        <D:multistatus xmlns:D="DAV:">
            <D:response>
        ...
        assert len(results) == 1
        assert results[0].href == "/test/"

    def test_check_response_ok(self):
        """check_response_ok should validate status codes."""
        protocol = CalDAVProtocol()

        assert protocol.check_response_ok(DAVResponse(200, {}, b""))
        assert protocol.check_response_ok(DAVResponse(201, {}, b""))
        assert not protocol.check_response_ok(DAVResponse(404, {}, b""))

        # Custom expected status
        assert protocol.check_response_ok(
            DAVResponse(404, {}, b""),
            expected_status=[404],
        )
