...
class ServerQuirkChecker():
    ...
    def set_flag(self, flag, value=True):
        if flag == 'rate_limited':
            ...
            if hasattr(self, '_default_calendar'):
                self._default_calendar.search = _delay_decorator(self._default_calendar.search, 60)
    ...
    def _try_make_calendar(self, cal_id, **kwargs):
        ...
        try:
            ...
            if (not cal or (self.flags_checked.get('non_existing_calendar_found' and len(events)==0))):
    ...
    def check_support(self):
        ...
        try:
            ...
            self.set_flag('no_scheduling', not self.client_obj.check_scheduling_support())
        except:
            pass
    ...
    def check_event(self):
        ...
        obj1 = cal.add_event(dtstart=datetime.datetime.now(), summary="Test event 1", categories=['foo','bar'], class_='CONFIDENTIAL', uid='check_event_1')
        obj2 = cal.add_event(dtstart=datetime.datetime.now(), summary="Test event 2", categories=['zoo','test'], uid='check_event_2')
        try: ## try-finally-block covering testing of obj1 and obj2
            ...
            if not self.flags_checked['text_search_not_working']:
                ...
                events = cal.search(summary='Test event 1', class_='CONFIDENTIAL', event=True)
                if len(events)==1:
        ...
        ## Recurring events
        try:
            yearly_time = cal.add_event(dtstart=datetime.datetime(2000, 1, 1, 0, 0), summary="Yearly timed event", uid='firework_event', rrule={'FREQ': 'YEARLY'})
            yearly_day = cal.add_event(dtstart=datetime.date(2000, 5, 1), summary="Yearly day event", uid='full_day_event', rrule={'FREQ': 'YEARLY'})
        except:
            ## should not be here
            import pdb; pdb.set_trace()
            raise
        try:
            try:
                events = cal.search(start=datetime.datetime(2001, 4, 1), end=datetime.datetime(2002,2,2), event=True)
                assert len(events)==2
            ...
            if not (self.flags_checked['no_recurring']):
                events = cal.search(start=datetime.datetime(2001, 4, 1), end=datetime.datetime(2002,2,2), event=True, expand='server')
                assert len(events)==2
    ...
    def report(self, verbose, json):
        ...
        if json:
            click.echo(dumps({'name': self.client_obj.server_name, 'url': str(self.client_obj.url), 'flags_checked': self.flags_checked, 'diff1': list(self.diff1), 'diff2': list(self.diff2)}, indent=4))
            click.echo()
            return
        if verbose is False:
            return
