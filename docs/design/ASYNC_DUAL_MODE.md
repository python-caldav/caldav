(This document was human-written)
...
* For methods containing significant amount of preparation logic (like, two or
  more code lines) before doing any IO, the
  `if self.is_async_client: return self._async_foo(...)`-logic
  should be moved to the last possible point in the method.
* For methods containing significant amount of processing logic after doing the IO,
  split the logic out in a `_post_foo`-method.

Now, some of the methods may return cached data if it exists, avoiding IO-operations.  A wrapper class method `BaseDAVClient._value_or_coroutine` has been made to deal with this so that async users can always expect to get a coroutine.
