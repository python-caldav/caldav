...
| `cal.get_object_by_uid(uid)` → `JMAPCalendarObject` | — | `CalendarEvent/query` + `CalendarEvent/get` (fetches all events in calendar, scans locally for UID match; see limitation below) |
| `cal.search(event=True, start=, end=, text=)` → `list[JMAPCalendarObject]` | `search_events(calendar_id, start, end, text)` | `CalendarEvent/query` + result reference into `CalendarEvent/get` in one round-trip |
| — | `get_event(event_id)` → `JMAPCalendarObject` | `CalendarEvent/get` — lower-level, takes JMAP event ID directly |
...
Events are returned as `JMAPCalendarObject` instances. This wrapper was
introduced so that JMAP objects "quack like" `caldav.CalendarObjectResource`
instances: code that calls `.get_icalendar_instance()` or `.parent` on a CalDAV
object can do the same on a JMAP object without modification.

The object exposes `.id` (server-assigned JMAP event ID), `.parent`
(the `JMAPCalendar` it belongs to), `get_data()` (raw JSCalendar dict — a dict,
not a string, because JMAP is JSON-native), `get_icalendar_instance()`
(converted `icalendar.Calendar`), `edit_icalendar_instance()` (context manager
for in-place editing), and `save()` (persists edits via `update_event`). Input
iCalendar strings are accepted by `create_event` and `update_event`; conversion
to/from JSCalendar happens inside the library. No typed dataclass attributes
mirror JMAP field names — that mapping belongs outside the caldav layer.

`save()` is sync-only. Async-backed calendars (`AsyncJMAPClient`) must call
`await client.update_event(obj.id, ical_str)` directly.

`add_event()` is a documented exception: it returns a plain `str` (the
server-assigned event ID), because `CalendarEvent/set` only echoes back the ID,
not the full object. A round-trip GET would be needed to return a full
`JMAPCalendarObject`; that is deferred.
...
In iCalendar, `VEVENT` and `VTODO` share almost the same property set and the
`icalendar` library treats them nearly interchangeably. JMAP is structurally
different: `Task` and `CalendarEvent` are separate top-level types defined in
separate specifications (`urn:ietf:params:jmap:tasks` vs.
`urn:ietf:params:jmap:calendars`), with separate method families, separate
capability URNs, and separate account capability checks. A server can implement
one without the other.

This is why `cal.search()` on a `JMAPCalendar` does not accept a `todo=True`
argument the way `caldav.Calendar.search()` does: there is no unified
`CalendarEvent+Task/query` method in the JMAP spec. Task search (`Task/query`)
is a separate operation that is not yet implemented.

| Client method | JMAP call |
|---------------|-----------|
| `get_task_lists()` → `list[dict]` | `TaskList/get` |
| `create_task(task_list_id, title, **kwargs)` → task_id | `Task/set` (create) |
| `get_task(task_id)` → `dict` | `Task/get` |
| `update_task(task_id, patch)` | `Task/set` (update) |
| `delete_task(task_id)` | `Task/set` (destroy) |

Task methods return raw dicts rather than typed dataclasses — consistent with the
library's design principle that typed field mapping belongs outside the caldav
layer.
...
kwargs for `create_task` use JMAP wire names: `description`, `due`, `start`,
`timeZone`, `estimatedDuration`, `percentComplete`, `progress`, `priority`.

### Internal structure

The method builders and response parsers live in `caldav.jmap._methods` (note
the leading underscore). This signals by PEP 8 convention that the entire layer
is internal plumbing — stable for the library's own use but not part of the
public API, and subject to change without a deprecation period. The `_` prefix
on the package name was chosen deliberately over per-function underscores so
that the signal is visible at the import path level without touching every
...
| `VEVENT` | `@type: "Event"` | Top-level type annotation per RFC 8984 §4.1.1 |
...
244 unit tests, zero network calls. They cover session parsing (including
...
24 integration tests: 17 against Cyrus IMAP via Docker on port 8802, 7 against
Stalwart via Docker on port 8809. The Cyrus tests exercise the full event
lifecycle: create, get, update, delete, search by date range and text,
incremental sync with `changes`, and the session account discovery path. 6 of
the 17 are async equivalents using `AsyncJMAPClient`. The Stalwart tests cover
calendar listing and basic event CRUD.
...
(via `start`), `before` (via `end`), `text`. Note: Stalwart rejects `inCalendars`
in `CalendarEvent/query` — the `calendar_id` argument is silently omitted from
queries sent to Stalwart, meaning results come from all calendars rather than
the scoped one (see Server Compatibility). This is a Stalwart bug, not a
deliberate design choice; the omission is a workaround until it's fixed upstream.
...
No integration tests for tasks yet — Cyrus doesn't implement
`urn:ietf:params:jmap:tasks`, and Stalwart task testing hasn't been wired up
beyond the basic Docker setup.
...
- **EventSource** — persistent HTTP connection (SSE); practical for long-lived
  server-side processes. Defined in RFC 8620 §7.3.
- **Web Push** — encrypted delivery to a third-party push gateway (FCM, APNs);
  practical for mobile and background delivery. Stalwart advertises this.

RFC 8887 also defines a WebSocket transport for JMAP as an alternative to
EventSource — same push semantics over a persistent WebSocket connection.

None of this is implemented. `get_objects_by_sync_token()` is the polling
alternative.
...
The raw JSCalendar dict returned by `get_event()` and `search_events()` carries
whatever `links` data the server returns, but the conversion layer doesn't map
iCalendar `ATTACH` properties to or from `links`. Neither upload nor download
is implemented.
...
| [JMAP Calendars](https://datatracker.ietf.org/doc/draft-ietf-jmap-calendars/) (draft-ietf-jmap-calendars-26) | In the **RFC Editor Queue** as of early 2026 (last updated Nov 2025; IANA expert reviews approved). IESG evaluation complete. Defines `Calendar/get/set/changes`, `CalendarEvent/get/set/query/changes/copy/parse`, `CalendarEventNotification`, `Principal/get/getAvailability`, `ParticipantIdentity`, and `PushSubscription`. |
...
| [RFC 9670](https://www.rfc-editor.org/rfc/rfc9670) — JMAP Sharing | Published (Nov 2024). Defines `Principal` (user, group, resource) and `ShareNotification` objects. Methods: `Principal/get`, `/changes`, `/set`, `/query`, `/queryChanges`; `ShareNotification/get`, `/changes`, `/set`, `/query`, `/queryChanges`. The framework that `Calendar.shareWith` builds on. |
| [RFC 8887](https://www.rfc-editor.org/rfc/rfc8887) — JMAP WebSocket | Published (Aug 2020). Defines a WebSocket binding for JMAP as an alternative to HTTP for real-time push. Not implemented. |
| [JSCalendar 2.0](https://datatracker.ietf.org/doc/draft-ietf-calext-jscalendarbis/) (draft-ietf-calext-jscalendarbis-15) | In AD Evaluation (rev. 15, Feb 2026; expires Aug 2026). Has passed WG Last Call; the responsible AD (Orie Steele) is in AD Followup — authors are responding to AD comments before IESG ballot. Obsoletes RFC 8984. Aligns type annotations and registry policy with JSContact; deprecates properties that conflict semantically with iCalendar. |
...
| [JMAP Tasks](https://www.ietf.org/archive/id/draft-ietf-jmap-tasks-06.html) | Specification at jmap.io only. The IETF draft (draft-ietf-jmap-tasks) expired September 2023 without progressing to RFC. The `urn:ietf:params:jmap:tasks` URN is defined at jmap.io, not in any RFC. This is the most spec-unstable part of the implementation. |
...
**Tested.** Docker setup at `tests/docker-test-servers/stalwart/`, port 8809,
testuser/testpass. 7 integration tests cover calendar listing and basic event
CRUD (create, get, update, delete, time-range search, iCalendar round-trip).

Two quirks discovered during integration:

- Stalwart rejects `inCalendars` in `CalendarEvent/query` — time-range queries
  must omit the calendar scope filter.
- `hostname: localhost` is required in `docker-compose.yml` or Stalwart
  advertises the Docker container ID in the JMAP session `apiUrl`, making all
  subsequent API calls fail. `session.py` rewrites the port when the host matches
  but the port differs (internal `:8080` vs host-mapped `:8809`).

Stalwart's implementation was partly funded by NLNet's NGI Zero grant — the same
program that funds python-caldav's JMAP work.

Features Stalwart has that Cyrus doesn't (not yet tested beyond basic CRUD):

- `CalendarEvent/parse` (configurable via `jmap.calendar.parse.max-items`)
- `urn:ietf:params:jmap:tasks` — task support
...
for batching query + get into one round-trip. Stalwart is now available for
testing (Docker setup in `tests/docker-test-servers/stalwart/`).
...
third-party push gateway needed. Once in place, the same infrastructure works
for any data type that has a `*/changes` method, including tasks and contacts.
Stalwart advertises Web Push support. Cyrus's push support status is not
confirmed (its JMAP implementation is described as a work in progress).

### Sharing

RFC 9670 (JMAP Sharing, published Nov 2024) defines the `Principal` object type
and the `ShareNotification` type. `Principal` represents a user, group, or
resource that can own or share data; `ShareNotification` records permission
changes sent to the sharer or sharee.

In practice for calendars: `Calendar.shareWith` maps a principal ID to rights;
`ParticipantIdentity` lists the current user's email addresses and display names
as the server knows them — the canonical way to identify which `participants`
entry in an event is the current user without fragile email-string matching.

Methods not yet implemented: `Principal/get`, `Principal/changes`,
`Principal/set`, `Principal/query`, `ShareNotification/get`,
...
RFC 9610 (JMAP for Contacts) is already published and Stalwart implements it.
Stalwart is now in the test matrix (Docker on port 8809), so contact methods
are technically testable today. `urn:ietf:params:jmap:contacts` is a separate
capability on the same JMAP session — same `apiUrl`, same `accountId`, no
additional auth. A `JMAPContactsClient` that wraps `AddressBook/get` and
`ContactCard/get/query` could be added without touching any existing code.
...
1. **Extract `caldav.jmap` into a standalone library** (tentatively `python-calendar-jmap`
   ...
   allow it to be versioned, packaged, and maintained independently, and would
   keep the `python-caldav` namespace focused on CalDAV.

2. **Create a protocol-agnostic `calendaring-client` library** that wraps both
   `DAVClient` (from `python-caldav`) and `JMAPClient` (from the extracted JMAP
   library) behind a single API. User code would declare what server it connects
   to and get calendar objects back without caring which protocol was used.

```python
# future, in a separate calendaring-client library
client = get_calendar_client(url="https://cal.example.com", username="alice", password="s3cr3t")
calendars = client.get_calendars()  # works regardless of whether the server speaks CalDAV or JMAP
```
...
Neither move has a timeline. The JMAP Calendars spec (draft-ietf-jmap-calendars)
is still in the RFC Editor Queue as of early 2026; waiting for it to be published
before committing to a stable public API is prudent.
