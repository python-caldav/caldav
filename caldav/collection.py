...
def _quote_url_path(url: str, features: Any = None) -> str:
    ...
    would be churn at best and a 404 at worst.  Only a server whose
    ``url.encode-at`` entry declares ``at-spelling: encoded`` - one where the
    literal ``@`` actually 404s - gets the ``@`` rewritten to ``%40``.  The
    support *level* is never consulted: ``quirk`` records that the server
    deviates, not how.  The netloc is never touched, so credentials embedded
...
def _sanitize_calendar_home_set_url(url: str | None, features: Any = None) -> str | None:
    ...
    Skipped for a server whose ``url.encode-at`` entry declares
    ``at-spelling: literal``, i.e. one known to 404 on the percent-encoded
    form; there the literal ``@`` has to survive into the request.  With
    nothing configured the historic unconditional quoting stands.
    """
    if url is None:
        return None
