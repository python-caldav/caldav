...
class CalendarObjectInfo:
    ...
    data: Optional[str]
    etag: Optional[str]
    component_type: Optional[str]  # "Event", "Todo", "Journal", "FreeBusy"
    extra_props: dict


def detect_component_type_from_string(data: str) -> Optional[str]:
    """
...
def detect_component_type_from_icalendar(ical_obj: Any) -> Optional[str]:
    """
...
def detect_component_type(data: Any) -> Optional[str]:
    """
    ...
    if hasattr(data, "split"):
        return detect_component_type_from_string(data)

    # Try icalendar object detection
    if hasattr(data, "subcomponents"):
        return detect_component_type_from_icalendar(data)

    return None


def generate_fake_sync_token(etags_and_urls: List[Tuple[Optional[str], str]]) -> str:
    """
...
def is_fake_sync_token(token: Optional[str]) -> bool:
    """
...
def normalize_result_url(result_url: str, parent_url: str) -> str:
    """
...
def should_skip_calendar_self_reference(result_url: str, calendar_url: str) -> bool:
    """
...
def process_report_results(
    ...
    for href, props in results.items():
        ...
        if should_skip_calendar_self_reference(href, calendar_url_normalized):
            continue
        ...
        component_type = detect_component_type(data)

        # Normalize URL
        normalized_url = normalize_result_url(href, calendar_url)
