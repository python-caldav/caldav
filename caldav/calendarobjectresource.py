...
def _quote_uid(uid: str, features: Any = None) -> str:
    ...
    percent-encoded by default, which is what this has always done, but a
    server whose ``url.encode-at`` entry declares ``at-spelling: literal`` -
    one known to 404 on ``%40`` - gets the literal ``@`` instead.
    """
