...
class AsyncDAVResponse:
    ...
    def _strip_to_multistatus(self) -> None:
        """Strip response to multistatus element if present."""
        if self.tree is not None and self.tree.tag.endswith("multistatus"):
            return
        if self.tree is not None:
            multistatus = self.tree.find(".//{*}multistatus")
            if multistatus is not None:
                self.tree = multistatus
