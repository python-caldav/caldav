...
class FeatureSet:
    ...
    broken when the server does unexpected things.  Possibly some of the things classified as "unsupported" today should rather be classified as "broken" (and possibly vice-versa).  TODO: look more into this and clean up.
    ungraceful means the server will come up with an error (which usually causes the library to raise an error).  Came to think, "ungraceful" may in some cases be the best handling as the client may catch the error and handle it in the best possible way - while support level "unsupported", "broken" and "fragile" often may involve data loss.
...
nextcloud = {
    ...
    ## reports one now), and this should go back to 'quirk' with that
    ## observation rather than to 'fragile'.
...
zimbra = {
    ...
    ## reproduced.  As for Cyrus and Nextcloud: 'full' rather than 'quirk',
    ## since a delay too small to observe cannot be told from none at all -
    ## an actual observation is what should put a 'quirk' here.
    'delete-calendar': {'support': 'full'},
